/**定义模块和变量**/
// import versionPath from './public/version.txt';
const exec = require('child_process').exec; //异步子进程
const execSync = require('child_process').execSync; //同步子进程
const fs = require('fs'); //文件读取模块
const path = require('path');
const _versionPath =  path.resolve(__dirname, '../dist');
const versionPath = _versionPath + '/version.txt'; //version路径
...
//     // fs.writeFileSync(`${buildPath}/${versionPath}`, fs.readFileSync(versionPath));
//     // fs.writeFileSync(`${path}.resolve(${__dirname}, './build')`, fs.readFileSync(versionPath));
// }
